 ...
 * Allocated strictly within components/ActivityDashboardPage/ActivityJournal/ nested schema.
 */
const JournalDayGroup = ({ dateLabel, logs = [], isTrainerView = false }) => {
    ...
    <div className="relative pb-12 font-sans" dir="rtl">
      ...
      <div className="sticky top-6 z-20 mb-10 mr-2 flex">
        <div className="inline-flex items-center gap-4 bg-white/40 backdrop-blur-2xl px-6 py-3 rounded-[1.5rem] shadow-2xl shadow-zinc-200/50 border border-white/80 group transition-all">
          <div className="w-8 h-8 rounded-xl bg-zinc-900 flex items-center justify-center text-[10px] shadow-lg select-none">
            📅
          </div>
      ...
      {/* Glassmorphism Timeline Axis Line */}
      <div className="absolute top-0 bottom-0 right-[34px] w-1 bg-gradient-to-b from-white/60 via-zinc-200/40 to-transparent backdrop-blur-sm pointer-events-none select-none" />

      {/* Logs Collection Stream Arena */}
        ...
        {logs.map((log, index) => {
          ...
          return (
            <React.Fragment key={log.id}>
              {/* Session / Context Header Card Node */}
              {isNewGroup && (
                <div className="relative flex items-center gap-4 mb-6 mt-10 animate-in fade-in slide-in-from-right-6 duration-700">
                  {/* Glowing Timeline Indicator Point Vector */}
                  <div className="absolute -right-[36px] w-5 h-5 rounded-full bg-white border-[3px] border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.5)] z-10 transition-transform hover:scale-125 pointer-events-none select-none" />
                  ...
                  <div className="flex items-center gap-3 bg-blue-500/5 backdrop-blur-md border border-blue-500/10 px-5 py-2.5 rounded-2xl shadow-inner">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] font-mono leading-none">
                        {log.workout_session_id ? "Workout Session" : "Single Log Entry"}
                      </span>
                      <span className="text-sm font-black text-zinc-900 tracking-tighter mt-1">
                    ...
                    {log.workout_session_id && (
                      <span className="text-[9px] font-bold text-zinc-400 mr-2 bg-white/80 px-2 py-0.5 rounded-md border border-white/60 font-mono">
