...
const PersonalInfo = ({ user }) => {
  ...
  const { setUser } = useAuth(); // To update the global logged-in user state
  ...
  const handleSave = async () => {
    try {
      ...
      // 2. Update Global Auth State so the UI (Header etc.) updates immediately
      setUser(updatedUser);
    ...
    return (
      <section style={{ border: '2px solid #007bff', padding: '15px', borderRadius: '8px', marginBottom: '20px', backgroundColor: '#f0f8ff' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          ...
          <input name="password" type="password" placeholder="סיסמה חדשה (אופציונלי)" value={formData.password} onChange={handleInputChange} style={{ gridColumn: 'span 2' }} />
        </div>
        <div style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
          <button onClick={handleSave} style={{ backgroundColor: '#28a745', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' }}>שמור שינויים</button>
          <button onClick={() => setIsEditing(false)} style={{ backgroundColor: '#6c757d', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '4px', cursor: 'pointer' }}>ביטול</button>
        </div>
      </section>
    );
  }
  ...
  return (
    <section style={{ border: '1px solid #ddd', padding: '15px', borderRadius: '8px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff' }}>
      <div>
        ...
        <p><strong>תפקיד:</strong> {user?.role === 'trainee' ? 'מתאמן' : user?.role}</p>
      </div>
      <button 
