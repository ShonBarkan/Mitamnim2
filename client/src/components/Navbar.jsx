...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-[100] w-full bg-white/60 backdrop-blur-2xl border-b border-white/60 px-8 py-4 font-sans select-none" dir="rtl">
      <div className="max-w-[1800px] mx-auto flex items-center justify-between">
        
        {/* Brand Identity & Core Navigation Links Suite */}
        <div className="flex items-center gap-10">
          {/* Arctic Mirror Logo Mark Badge */}
          <div className="flex items-center gap-3 ml-6 select-none pointer-events-none">
            <div className="w-8 h-8 bg-zinc-900 rounded-2xl flex items-center justify-center text-xs text-white font-black shadow-2xl shadow-zinc-900/20">
              M
            </div>
            <span className="text-[10px] font-black tracking-[0.3em] uppercase text-zinc-900 hidden lg:block font-mono">
              Mitamnim v2
            </span>
          </div>
        ...
        {/* User Context Metadata & Infrastructure Terminals */}
  ...
  <Link 
    ...
    className={`px-5 py-2.5 rounded-[1.2rem] text-[11px] font-black uppercase tracking-tight transition-all duration-300 whitespace-nowrap active:scale-95 ${
      active 
        ...
        : subtle 
          ? 'text-zinc-400 hover:text-zinc-900 hover:bg-white/80 border border-transparent'
          : 'text-zinc-500 hover:text-zinc-900 hover:bg-white/80 border border-transparent hover:border-white/80 shadow-sm hover:shadow-md'
    }`}
  >
    {children}
  </Link>
