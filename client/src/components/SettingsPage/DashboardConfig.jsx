...
const DashboardConfig = () => {
  const { dashboardConfigs, loading, refreshAllConfigs, addDashboardConfig, updateDashboardConfig, removeDashboardConfig } = useStats();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        showToast("חוקיות תצוגת הדשבורד עודכנה", "success");
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mr-2">כותרת המדד (לדוגמה: שיא קבוצתי)</label>
              <input type="text" name="title" placeholder="Leaderboard Title" value={formData.title} onChange={handleInputChange} className="w-full bg-white/60 border border-white/80 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none focus:ring-4 focus:ring-zinc-900/5 transition-all" />
            </div>

            {/* Flat Exercise Selector mapping names */}
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mr-2">תרגיל היעד</label>
              <select name="exercise_name" value={formData.exercise_name} onChange={handleInputChange} className="w-full bg-white/60 border border-white/80 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none focus:ring-4 focus:ring-zinc-900/5 appearance-none transition-all">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mr-2">הפרמטר המוביל למדידה</label>
              <select name="parameter_id" value={formData.parameter_id} onChange={handleInputChange} className="w-full bg-white/60 border border-white/80 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none focus:ring-4 focus:ring-zinc-900/5 appearance-none transition-all">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mr-2">פורמט ויזואליזציה</label>
              <select name="display_type" value={formData.display_type} onChange={handleInputChange} className="w-full bg-white/60 border border-white/80 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none focus:ring-4 focus:ring-zinc-900/5 appearance-none transition-all">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mr-2">סדר עדיפות בלוח</label>
              <input type="number" min="1" name="sort_order" value={formData.sort_order} onChange={handleInputChange} className="w-full bg-white/60 border border-white/80 rounded-2xl px-5 py-3.5 text-sm font-bold outline-none focus:ring-4 focus:ring-zinc-900/5 transition-all" />
            </div>
          ...
          <div className="flex justify-end gap-3 border-t border-zinc-900/5 pt-4">
            {editingId && (
              <button type="button" onClick={resetForm} className="px-6 py-2.5 bg-white/80 border border-zinc-200 rounded-xl text-zinc-500 font-bold text-xs hover:bg-white transition-all">
                ביטול
              </button>
            )}
            <button type="submit" className="px-8 py-2.5 bg-zinc-900 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-md hover:bg-zinc-800 transition-all active:scale-95">
      ...
      <div className="border border-zinc-100 rounded-2xl overflow-hidden shadow-sm bg-white/20">
        <table className="w-full text-right border-collapse">
          ...
          <tbody className="divide-y divide-zinc-100">
            {loading ? (
              <tr>
                <td colSpan="6" className="p-10 text-center text-xs font-bold text-zinc-400 uppercase animate-pulse">Syncing Board Config Layers...</td>
              </tr>
            ) : dashboardConfigs.length === 0 ? (
              <tr>
                <td colSpan="6" className="p-10 text-center text-xs font-bold text-zinc-300 italic">דף הבית ריק ממדדים פומביים. הגדר תצוגה ראשונה למעלה.</td>
              </tr>
            ) : (
              [...dashboardConfigs]
                .sort((a, b) => a.sort_order - b.sort_order)
                .map(config => (
                  <tr key={config.id} className="group transition-all hover:bg-white/40">
                    <td className="px-6 py-4 text-center">
                      <span className="bg-zinc-100 text-zinc-700 font-mono text-xs px-2.5 py-1 rounded-md border border-zinc-200/60 font-black">
                        #{config.sort_order}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${
                        config.display_type === 'leaderboard' ? 'bg-amber-100 text-amber-700' :
                        config.display_type === 'chart' ? 'bg-blue-100 text-blue-700' : 'bg-emerald-100 text-emerald-700'
                      }`}>
                    ...
                    <td className="px-6 py-4 text-left">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button type="button" onClick={() => startEdit(config)} className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition-colors active:scale-90">✏️</button>
                        <button type="button" onClick={() => handleDelete(config.id)} className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition-colors active:scale-90">🗑️</button>
                      </div>
                    </td>
                  </tr>
                ))
